...
class VAE(nn.Module):
    ...
    def calculate_size_after_convolutions(self, size):
        size = size // 2**len(self.encoder)  # Assuming all layers are stride 2
    ...
    def forward(self, x):
        ...
        mu = self.fc_mu(encoded.view(encoded.size(0), -1))
        log_var = self.fc_var(encoded.view(encoded.size(0), -1))
        z = self.reparameterize(mu, log_var)
        decoded = self.decoder(self.decoder_input(z).view(-1, self.h_dim_last,
                                                         int((self.flattened_size/self.h_dim_last)**0.5),
                                                         int((self.flattened_size/self.h_dim_last)**0.5)))
...
class FrameProcessor:
    ...
    def set_cnn_model(self):
        ...
        hidden_dims = [64, 128, 256, 512]  # Example hidden dimensions for the encoder/decoder
